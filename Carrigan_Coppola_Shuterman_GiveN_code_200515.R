...
GiveN_subset$GN_sm_2cat <- factor(ifelse(GiveN_subset$GN_ALL_sm_fac == 6, "CP-knower", "NOT CP-knower"), levels = c("NOT CP-knower", "CP-knower"))
...
## BEESWARM BOXPLOT FOR GN-2-cat (CP/Non) w/dot colors by Language Modality & Language Timing
...
ggplot(data=GiveN_subset, mapping = aes(x=GN_sm_2cat, y=Age)) + geom_boxplot() + geom_dotplot(mapping=aes(fill=Language_Timing, color=Language_Timing), method = "dotdensity", binaxis = "y", position = jitter, stackdir = "center", dotsize=0.5, stackratio = 1.25) + theme(text = element_text(size=10), axis.title.x=element_blank())  + ylab("Age (Years)") + scale_fill_manual(name = "Language\nTiming", breaks=c("Later", "Early"), values=c("firebrick3", "grey37"), labels=c("Later (83)", "Early (93)")) + scale_color_manual(name = "Language\nTiming", breaks=c("Later", "Early"), values=c("firebrick3", "grey37"), labels=c("Later (83)", "Early (93)"))
...
#N for code below is: summary() values - NA values from table()
ggplot(data=GiveN_subset, mapping = aes(x=GN_sm_2cat, y=Age)) + geom_boxplot() + geom_dotplot(mapping=aes(fill=Language_Modality, color=Language_Modality), binaxis = "y", position = jitter, stackdir = "center", dotsize=0.5, stackratio = 1.25) + theme(text = element_text(size=10), axis.title.x=element_blank())  + ylab("Age (Years)") + scale_fill_manual(name = "Language Modality", values=c("tan2", "forestgreen"), breaks=c("English", "ASL"), labels=c("English (105)", "ASL (71)")) + scale_color_manual(name = "Language Modality", values=c("tan2", "forestgreen"), breaks=c("English", "ASL"), labels=c("English (105)", "ASL (71)"))




# of kids who got different proportion correct values on GN large
summary(GiveN_subset$Language_Timing)
